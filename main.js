...
class Awattar extends utils.Adapter {
  ...
  async fetchAwattarData() {
    ...
    if (isNaN(parseInt(loadingThresholdEnd))) {
      return this.log.error("loadingThresholdEnd NaN");
    }

    const heute = new Date();
    const loadingThresholdStartDateTime = new Date(
      heute.getFullYear(),
      heute.getMonth(),
      heute.getDate(),
      parseInt(loadingThresholdStart),
      0,
      0,
    );
    const loadingThresholdEndDateTime = new Date(
      heute.getFullYear(),
      heute.getMonth(),
      heute.getDate() + 1,
      parseInt(loadingThresholdEnd),
      0,
      0,
    );

    const epochToday = new Date(
      heute.getFullYear(),
      heute.getMonth(),
      heute.getDate(),
    ).getTime();
    ...
    const urlEpoch = url.concat(
      "?start=",
      epochToday.toString(),
      "&end=",
      epochTomorrow.toString(),
    );
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        ...
        this.log.warn(
          ...
          } response from local sensor with content: ${JSON.stringify(
            error.response.data,
          )}`,
        );
    ...
    this.log.debug(
      `received data (${response.status}): ${JSON.stringify(content)}`,
    );
    ...
    for (let i = 0; i < array.length; i++) {
      ...
      await Promise.all([
        ...
        this.setStateAsync(
          `${stateBaseName}nettoPriceKwh`,
          nettoPriceKwh,
          true,
        ),
        this.setStateAsync(
          `${stateBaseName}bruttoPriceKwh`,
          bruttoPriceKwh,
          true,
        ),
        this.setStateAsync(
          `${stateBaseName}totalPriceKwh`,
          totalPriceKwh,
          true,
        ),
      ]);
    ...
    for (let k = 0; k < sortedArray.length; k++) {
      ...
      if (
        start >= loadingThresholdStartDateTime &&
        end < loadingThresholdEndDateTime
      ) {
    ...
    this.stopTimeout = this.setTimeout(() => {
  ...
  onUnload(callback) {
    try {
      if (this.stopTimeout) {
        this.clearTimeout(this.stopTimeout);
